...
on the left half of the frame."
  ...
  (let ((other-buffer (and (next-window) (window-buffer (next-window)))))
    ...
    (when other-buffer
      (set-window-buffer (next-window) other-buffer))))
(global-set-key (kbd "C-c 2") 'split-window-horizontally-instead)
(global-set-key (kbd "C-c 3") 'split-window-vertically-instead)
...
(global-set-key (kbd "C-x w") 'sanityinc/split-window)
