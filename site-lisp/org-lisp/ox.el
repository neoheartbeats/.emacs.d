...
implementation list is returned."
  (let ((initial-features (or features (plist-get info :features)))
    ...
    ;; Sort by :order.  Modifications past this point should be stable.
    (setq current-implementations
    ...
    (let ((processing t)
          (confirmed-features
           (cl-remove-if
            ...
            (or features (plist-get info :features))))
...
on what this entails."
  (let ((feature-indicies
    ...
    (dolist (impl implementations)
      ;; FIXME use `ensure-list' as soon as possible.
      (let ((before (plist-get (cdr impl) :before))
            (after (plist-get (cdr impl) :after)))
        (when (and before (symbolp before))
        ...
        (while (and deps (not new-dep-found))
          (if (not (assq (car deps) adj-list))
            ...
            (when (assq (car deps) node-stack)
              (error "Found a cycle in the feature dependency graph: %S"
                     (cons (car deps)
